...
const addUserBills = `-- name: AddUserBills :many
INSERT INTO user_bills (bill_id, group_id, lend_user_id, borrow_user_id, amount)
SELECT unnest($1::bigint[]) AS bill_id, unnest($2::bigint[]) AS group_id, unnest($3::bigint[]) AS lend_user_id, unnest($4::bigint[]) AS borrow_user_id, unnest($5::int[]) AS amount
RETURNING bill_id, group_id, lend_user_id, borrow_user_id, amount
`

type AddUserBillsParams struct {
	BillID       []int64 `json:"bill_id"`
	GroupID      []int64 `json:"group_id"`
	LendUserID   []int64 `json:"lend_user_id"`
	BorrowUserID []int64 `json:"borrow_user_id"`
...
func (q *Queries) AddUserBills(ctx context.Context, arg AddUserBillsParams) ([]AddUserBillsRow, error) {
	rows, err := q.db.Query(ctx, addUserBills,
		arg.BillID,
		arg.GroupID,
		arg.LendUserID,
		arg.BorrowUserID,
	...
	defer rows.Close()
	var items []AddUserBillsRow
	for rows.Next() {
		var i AddUserBillsRow
		if err := rows.Scan(
			&i.BillID,
			&i.GroupID,
			&i.LendUserID,
