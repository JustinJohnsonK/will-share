...
func (s *GroupService) Create(c context.Context, group store.CreateGroupParams) (store.Group, error) {
	i, err := s.db.CreateGroup(c, group)
	return i, err
}

func (s *GroupService) Get(c context.Context, group_id int64) (store.Group, error) {
...
func (s *GroupService) AddUserToGroup(c context.Context, userGroup store.AddUserToGroupParams) (store.GroupUser, error) {
